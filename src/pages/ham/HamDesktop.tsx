...
export default function Ham() {
  ...
  const onUpdate = (duration: number, speedTicks?: number) => {
    ...
    if (speedTicks) {
      ...
      nameTagUpdate(Math.round(((rot + 180) / 180) * 6));
    }
  };
  const nameTagUpdate = (id: number) => {
    nameTagRef.current.forEach((tag, i) => {
      if (i === id) {
        gsap.to(tag, {
          scale: 1,
          x: 0,
    ...
    const startAnim = () => {
      ...
      gsap
        ...
        .fromTo(
          ...
          { rotation: 20, duration: mainDuration },
        )
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.speedometer} ref={mainSpeedometerRef}>
        ...
        {/* Speed dial */}
        <div className={styles.speedDial} ref={speedDialRef}>
          <svg
            width="12"
            height="200"
            viewBox="0 0 12 200"
        ...
        {nameTags.map(({ id, label, side, mouseEnter, animId, onClick }) => (
          <div
            ...
            onMouseLeave={() => {
              if (hoverTimeoutRef.current && startHovering) {
