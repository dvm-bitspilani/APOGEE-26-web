...
export default function CameraControllerLeva() {
  ...
  const { x, y, z, rotx, roty, rotz } = useControls("Camera", {
    x: { value: 0, min: -50, max: 50, step: 0.04 },
    y: { value: 3.2, min: -50, max: 50, step:0.04},
    z: { value: -2, min: -50, max: 50, step: 0.04 },
    rotx: { value: 0, min: -Math.PI, max: Math.PI, step: 0.1 },
    roty: { value: -Math.PI/2, min: -Math.PI, max: Math.PI, step: 0.1 },
